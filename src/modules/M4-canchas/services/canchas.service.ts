...
export class CanchasService {
    ...
    async crearCancha(dto: CrearCanchaDto) {
        ...
        const tipoValido = await this.canchasRepository.existeTipoCancha(dto.id_cancha_tipo);
        if (!tipoValido) {
            throw new BadRequestException(`El tipo de cancha con ID ${dto.id_cancha_tipo} no existe.`);
        }
